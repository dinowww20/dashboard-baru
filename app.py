...
# CSS: Memaksa Latar Belakang dan Kontras Hitam Pekat
st.markdown("""
    <style>
    /* Background abu-abu terang untuk memisahkan kartu putih */
    .stApp {
    ...
    /* Memaksa semua teks di Streamlit menjadi Hitam */
...
# Palet Warna Tegas (Tidak Nyaru)
nps_colors = {
    'Promoter': '#10B981', # Hijau Terang
    'Passive': '#F59E0B',  # Kuning/Orange (Lebih jelas dari abu-abu)
    'Detractor': '#EF4444' # Merah Terang
}
color_primary = '#1E3A8A'   # Navy Gelap Solid
color_secondary = '#3B82F6' # Biru Terang Solid

# Fungsi Pemaksa Label Hitam Pekat pada Plotly
def elite_layout(fig, title=""):
    fig.update_layout(
        ...
        # SETTING UTAMA: Paksa semua font global di chart jadi HITAM
        font=dict(color='#000000', size=13, weight='bold'),
        hoverlabel=dict(bgcolor="#1E3A8A", font_color="#FFFFFF", font_size=13)
    )
    fig.update_xaxes(
        showgrid=True, gridwidth=1, gridcolor='#D1D5DB', zeroline=False, 
        title_font=dict(color='#000000', weight='bold'), 
...
st.markdown("<h2 style='text-align: center; margin-bottom: 30px;'>Dashboard Analitik Rumah Sakit</h2>", unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Executive Summary", "Service Deep Dive", "Loyalty & Retention", "Voice of Customer", "Text Analytics"
])
...
with tab1:
    ...
    t1_c1, t1_c2 = st.columns([2.5, 1.5])
    with t1_c1:
        trend_data = df_filtered.groupby('Date').agg(Total=('NPS', 'count'), NPS_Avg=('NPS', 'mean')).reset_index()
        fig_trend = go.Figure()
        fig_trend.add_trace(go.Bar(x=trend_data['Date'], y=trend_data['Total'], name='Kunjungan', marker_color='#93C5FD', yaxis='y1'))
        fig_trend.add_trace(go.Scatter(x=trend_data['Date'], y=trend_data['NPS_Avg'], name='NPS Rata-rata', mode='lines+markers', line=dict(color=color_primary, width=4), yaxis='y2'))
        fig_trend = elite_layout(fig_trend, "Dinamika Kunjungan & Skor Kepuasan")
        fig_trend.update_layout(yaxis=dict(showgrid=False), yaxis2=dict(overlaying='y', side='right', range=[0, 10], showgrid=False))
        # Memaksa teks legend warna hitam
        fig_trend.update_layout(legend=dict(font=dict(color="#000000")))
        st.plotly_chart(fig_trend, use_container_width=True)
        
    with t1_c2:
        nps_counts = df_filtered['NPS_Category'].value_counts().reset_index()
        fig_donut = px.pie(nps_counts, values='count', names='NPS_Category', hole=0.6, color='NPS_Category', color_discrete_map=nps_colors)
        fig_donut = elite_layout(fig_donut, "Distribusi Segmen Pelanggan")
        # Memaksa teks persen di dalam donut warna HITAM tebal
        fig_donut.update_traces(textposition='outside', textinfo='percent+label', textfont=dict(color='#000000', size=14, weight='bold'), marker=dict(line=dict(color='#000000', width=1.5)))
    ...
    with t1_r2_c1:
        ...
        fig_lead = px.bar(branch_nps, x='NPS', y='Branch', orientation='h', color_discrete_sequence=[color_primary])
        fig_lead = elite_layout(fig_lead, "Peringkat Kinerja Fasilitas")
        # Paksa teks bar (jika ada) jadi hitam
        fig_lead.update_traces(textfont_color='#000000')
        st.plotly_chart(fig_lead, use_container_width=True)
        
    with t1_r2_c2:
        # Treemap pakai RdYlGn (Merah-Kuning-Hijau) super kontras
        fig_tree = px.treemap(df_filtered, path=[px.Constant("Semua Pasien"), 'Branch', 'Gender', 'NPS_Category'], color='NPS', color_continuous_scale='RdYlGn')
        fig_tree = elite_layout(fig_tree, "Peta Hierarki Demografi")
        # Paksa teks di dalam kotak warna HITAM
...
with tab2:
    ...
    with t2_c1:
        ...
        fig_radar.update_traces(fill='toself', line_color=color_secondary, fillcolor='rgba(59, 130, 246, 0.3)', line_width=3)
        fig_radar = elite_layout(fig_radar, "Spektrum Kinerja Layanan")
        fig_radar.update_layout(polar=dict(radialaxis=dict(range=[1, 5], tickfont=dict(color="#000000")), angularaxis=dict(tickfont=dict(color="#000000"))))
        st.plotly_chart(fig_radar, use_container_width=True)
        
    with t2_c2:
        corr_data = df_filtered[services + ['NPS']].corr()['NPS'].drop('NPS').sort_values(ascending=True)
        fig_driver = px.bar(corr_data, orientation='h', color_discrete_sequence=[color_secondary])
        fig_driver = elite_layout(fig_driver, "Faktor Pendorong Kepuasan")
        fig_driver.update_traces(textfont_color='#000000')
        st.plotly_chart(fig_driver, use_container_width=True)

    heatmap_data = df_filtered.groupby('Branch')[services].mean()
    # Heatmap Blues: Aman dan kontras
    fig_heat = px.imshow(heatmap_data.T, color_continuous_scale='Blues', text_auto=".1f", aspect="auto")
    fig_heat = elite_layout(fig_heat, "Matriks Kinerja Layanan")
    # Teks heatmap paksa hitam/putih sesuai background (Plotly auto, tapi layout hitam bantu sumbu)
    ...
    with t2_r3_c2:
        ...
        fig_dn = go.Figure(data=[
            go.Bar(name='Dokter', x=doc_nurse['Branch'], y=doc_nurse['Doctor Consultation'], marker_color=color_primary),
            go.Bar(name='Perawat', x=doc_nurse['Branch'], y=doc_nurse['Nurse Service'], marker_color=color_secondary)
        ])
        fig_dn = elite_layout(fig_dn, "Komparasi Evaluasi Medis")
        fig_dn.update_layout(barmode='group', yaxis=dict(range=[1, 5]), legend=dict(font=dict(color="#000000")))
...
with tab3:
    ...
    with t3_c1:
        ...
        fig_bub = px.scatter(bubble_data, x="CSI", y="Loyalty", size="CES", color="Branch", hover_name="Branch", size_max=35)
        fig_bub.update_traces(marker=dict(line=dict(width=1.5, color='#000000'))) # Outline hitam
        fig_bub = elite_layout(fig_bub, "Matriks Kuadran (Kepuasan vs Retensi)")
        fig_bub.add_vline(x=bubble_data['CSI'].mean(), line_dash="dash", line_color="#000000", line_width=2)
        fig_bub.add_hline(y=bubble_data['Loyalty'].mean(), line_dash="dash", line_color="#000000", line_width=2)
        fig_bub.update_layout(legend=dict(font=dict(color="#000000")))
    ...
    with t3_r2_c1:
        ...
        fig_al = px.bar(age_loyalty, x='Age_Group', y='Loyalty', color_discrete_sequence=[color_secondary])
    ...
    with t3_r2_c2:
        ...
        fig_ces = elite_layout(fig_ces, "Beban Upaya Pelanggan")
        fig_ces.update_layout(legend=dict(font=dict(color="#000000")))
    ...
    with t3_r2_c3:
        ...
        fig_tl.update_traces(line=dict(color=color_primary, width=3))
...
with tab4:
    ...
    with t4_c1:
        ...
        fig_hour.update_traces(line=dict(color=color_primary, width=3))
        fig_hour = elite_layout(fig_hour, "Fluktuasi Sentimen per Jam")
    ...
    st.markdown("#### Tinjauan Data Kualitatif")
    if 'Improvement_Feedback' in df_filtered.columns:
        cat = st.radio("Saring Sentimen:", ["Semua Komentar", "Hanya Detractor (Kecewa)"], horizontal=True)
        vocab_df = df_filtered if cat == "Semua Komentar" else detractors
        st.dataframe(
    ...
    fig_stack = elite_layout(fig_stack, "Proporsi Sentimen per Cabang")
    fig_stack.update_layout(barmode='stack', legend=dict(font=dict(color="#000000")))
    st.plotly_chart(fig_stack, use_container_width=True)

# ----------------- TAB 5 -----------------
with tab5:
    st.markdown("#### Analisis Semantik & Kata Kunci")
    
    if 'Improvement_Feedback' in df_filtered.columns:
        all_text = " ".join(df_filtered['Improvement_Feedback'].dropna().astype(str).tolist()).lower()
        if len(all_text.strip()) > 0:
            t5_c1, t5_c2 = st.columns(2)
            
            with t5_c1:
                st.markdown("**Word Cloud (Seluruh Pasien)**")
                wordcloud_all = WordCloud(width=800, height=500, background_color='#ffffff', colormap='Blues', max_words=100).generate(all_text)
            ...
            with t5_c2:
                st.markdown("**Word Cloud (Pasien Kecewa)**")
                ...
                if len(detractor_text.strip()) > 0:
                    wordcloud_det = WordCloud(width=800, height=500, background_color='#ffffff', colormap='Reds', max_words=100).generate(detractor_text)
            ...
            if words:
                ...
                fig_bar_words = px.bar(
                    ...
                    text='Frekuensi Aktual', color_discrete_sequence=[color_secondary]
                )
                fig_bar_words = elite_layout(fig_bar_words, "Distribusi Kata Teratas")
