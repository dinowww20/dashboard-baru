...
# CSS Elite & Kontras Sempurna
st.markdown("""
    <style>
    /* Latar belakang abu-abu kebiruan lembut agar kartu putih sangat menonjol */
    .stApp {
        background-color: #F0F4F8;
    }
    
    /* Kartu putih bersih dengan bayangan elegan */
    .metric-card {
    ...
    /* Warna teks dipertajam agar kontras */
    .metric-title {
        color: #475569;
        font-size: 13px;
        font-weight: 700;
    ...
    h1, h2, h3, h4, p, span, div, label {
        color: #0F172A; /* Memaksa teks label menjadi hitam elegan */
        font-family: 'Helvetica Neue', sans-serif;
    }
    hr {
        border-color: #CBD5E1;
...
# Palet Warna Elite (Dinaikkan saturasinya sedikit agar tidak nyaru dengan putih)
nps_colors = {
    'Promoter': '#10B981', # Vibrant Emerald
    'Passive': '#94A3B8',  # Slate Gray
    'Detractor': '#EF4444' # Vibrant Crimson
}
primary_color = '#1E3A8A'   # Deep Royal Navy
secondary_color = '#0284C7' # Ocean Blue

# Fungsi Layout Universal
def elite_layout(fig, title=""):
    fig.update_layout(
        title=dict(text=title, font=dict(size=17, color='#0F172A', family='Helvetica Neue', weight='bold')),
        template="plotly_white",
        plot_bgcolor='rgba(0,0,0,0)',
        paper_bgcolor='rgba(0,0,0,0)',
        margin=dict(t=60, b=30, l=30, r=30),
        font=dict(color='#0F172A', size=12, weight='bold'),
        hoverlabel=dict(bgcolor="#0F172A", font_size=13, font_family="Helvetica Neue", font_color="#FFFFFF")
    )
    # Gridlines dibuat abu-abu tipis agar grafik mudah dibaca tanpa mengganggu
...
with tab1:
    ...
    with t1_r1_c1:
        ...
        fig_trend = go.Figure()
        fig_trend.add_trace(go.Bar(x=trend_data['Date'], y=trend_data['Total'], name='Volume Kunjungan', marker_color='#93C5FD', opacity=0.8, yaxis='y1'))
        fig_trend.add_trace(go.Scatter(x=trend_data['Date'], y=trend_data['NPS_Avg'], name='NPS Rata-rata', mode='lines+markers', line=dict(color=primary_color, width=4), yaxis='y2'))
    ...
    with t1_r2_c1:
        ...
        fig_lead = px.bar(branch_nps, x='NPS', y='Branch', orientation='h', color='NPS', color_continuous_scale='Blues')
        fig_lead = elite_layout(fig_lead, "Peringkat Kinerja Fasilitas")
        fig_lead.update_layout(coloraxis_showscale=False)
        st.plotly_chart(fig_lead, use_container_width=True)
    with t1_r2_c2:
        fig_tree = px.treemap(df_filtered, path=[px.Constant("Semua Pasien"), 'Branch', 'Gender', 'NPS_Category'], color='NPS', color_continuous_scale='Blues')
...
with tab2:
    ...
    with t2_r1_c1:
        ...
        fig_radar.update_traces(fill='toself', line_color=secondary_color, fillcolor='rgba(2, 132, 199, 0.2)', line_width=3)
    ...
    with t2_r3_c1:
        fig_box = px.box(df_filtered, x='Branch', y='Waiting Time', color='Branch', color_discrete_sequence=px.colors.qualitative.Pastel)
...
with tab3:
    ...
    with t3_r1_c1:
        ...
        fig_bub = px.scatter(bubble_data, x="CSI", y="Loyalty", size="CES", color="Branch", hover_name="Branch", size_max=35, color_discrete_sequence=px.colors.qualitative.Set2)
        fig_bub.update_traces(marker=dict(line=dict(width=1, color='#0F172A')))
        fig_bub = elite_layout(fig_bub, "Matriks Kuadran (Kepuasan vs Retensi)")
        fig_bub.add_vline(x=bubble_data['CSI'].mean(), line_dash="dash", line_color="#94A3B8", line_width=2)
        fig_bub.add_hline(y=bubble_data['Loyalty'].mean(), line_dash="dash", line_color="#94A3B8", line_width=2)
    ...
    if 'Improvement_Feedback' in df_filtered.columns:
        ...
        if len(all_text.strip()) > 0:
            ...
            with t5_r1_c1:
                ...
                wordcloud_all = WordCloud(width=800, height=500, background_color='#ffffff', colormap='ocean', max_words=100, contour_width=0).generate(all_text)
            ...
            with t5_r1_c2:
                ...
                if len(detractor_text.strip()) > 0:
                    wordcloud_det = WordCloud(width=800, height=500, background_color='#ffffff', colormap='Reds', max_words=100, contour_width=0).generate(detractor_text)
            ...
            if words:
                ...
                fig_bar_words = px.bar(
                    ...
                    color='Frekuensi Aktual', color_continuous_scale='Blues', text='Frekuensi Aktual'
                )
